class Solution {
    ...
    private int helper(int[] nums, int[] multipliers, int left, int index, Integer[][] dp) {
        ...
        int res = Math.max(nums[left] * multipliers[index] + helper(nums, multipliers, left + 1, index + 1, dp), nums[right] * multipliers[index] + helper(nums, multipliers, left, index + 1, dp));
